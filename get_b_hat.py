...
class MST(nn.Module):
    ...
    def forward(self, x, y, z):
        ...
        # Embedding
        fea = self.embedding(x) if y is None else self.embedding(torch.cat([x, y, z], dim=1))
...
if __name__ == "__main__":
    trans_net = MST(in_dim=9, out_dim=3, dim=30, stage=3, num_blocks=[1, 1, 1])
    result = trans_net(torch.randn(2, 3, 384, 384), torch.randn(2, 3, 384, 384),torch.randn(2, 3, 384, 384))
    print(result.shape)
